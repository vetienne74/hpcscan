...
//-------------------------------------------------------------------------------------------------------
// This grid is activated with command line option -testMode Custom
// Derived class from Grid
// Can be used as a template for your own needs (target CPU)
//-------------------------------------------------------------------------------------------------------
...
Grid_Custom::Grid_Custom(Grid_type gridTypeIn) : Grid(gridTypeIn)
{
	printDebug(MID_DEBUG, "IN Grid_Custom::Grid_Custom");

	gridMode = GRID_MODE_CUSTOM ;

	printDebug(MID_DEBUG, "OUT Grid_Custom::Grid_Custom");
}

//-------------------------------------------------------------------------------------------------------

Grid_Custom::Grid_Custom(Grid_type gridTypeIn, Dim_type dimIn,
...
void Grid_Custom::info(void)
{
	printDebug(FULL_DEBUG, "IN Grid_Custom::info");
...
Rtn_code Grid_Custom::FD_D1_N1(Point_type pType, const Grid& Wgrid, Myint fdOrder)
{
	printDebug(MID_DEBUG, "IN Grid_Custom::FD_D1_N1");

	// check grids are same size
	if (this->sameSize(Wgrid) != true)
	{
		printError("Grid_Custom::FD_D1_N1, grids have not same size") ;
		return(RTN_CODE_KO) ;
	}
...
#pragma omp simd
	...
	printDebug(MID_DEBUG, "OUT Grid_Custom::FD_D1_N1");
	return(RTN_CODE_OK) ;
}

//-------------------------------------------------------------------------------------------------------

Rtn_code Grid_Custom::FD_D1_N2(Point_type pType, const Grid& Wgrid, Myint fdOrder)
{
	printDebug(MID_DEBUG, "IN Grid_Custom::FD_D1_N2");

	// check grids are same size
	if (this->sameSize(Wgrid) != true)
	{
		printError("Grid_Custom::FD_D1_N2, grids have not same size") ;
		return(RTN_CODE_KO) ;
	}
...
#pragma omp simd
	...
	printDebug(MID_DEBUG, "OUT Grid_Custom::FD_D1_N2");
	return(RTN_CODE_OK) ;
}

//-------------------------------------------------------------------------------------------------------

Rtn_code Grid_Custom::FD_D1_N3(Point_type pType, const Grid& Wgrid, Myint fdOrder)
{
	printDebug(MID_DEBUG, "IN Grid_Custom::FD_D1_N3");

	// check grids are same size
	if (this->sameSize(Wgrid) != true)
	{
		printError("Grid_Custom::FD_D1_N3, grids have not same size") ;
		return(RTN_CODE_KO) ;
	}
...
#pragma omp simd
	...
	printDebug(MID_DEBUG, "OUT Grid_Custom::FD_D1_N3");
	return(RTN_CODE_OK) ;
}

//-------------------------------------------------------------------------------------------------------

Rtn_code Grid_Custom::FD_D2_N1(Point_type pType, const Grid& Wgrid, Myint fdOrder)
{
	printDebug(MID_DEBUG, "IN Grid_Custom::FD_D2_N1");

	// check grids are same size
	if (this->sameSize(Wgrid) != true)
	{
		printError("Grid_Custom::FD_D2_N1, grids have not same size") ;
		return(RTN_CODE_KO) ;
	}
...
#pragma omp simd
	...
	printDebug(MID_DEBUG, "OUT Grid_Custom::FD_D2_N1");
	return(RTN_CODE_OK) ;
}

//-------------------------------------------------------------------------------------------------------

Rtn_code Grid_Custom::FD_D2_N2(Point_type pType, const Grid& Wgrid, Myint fdOrder)
{
	printDebug(MID_DEBUG, "IN Grid_Custom::FD_D2_N2");

	// check grids are same size
	if (this->sameSize(Wgrid) != true)
	{
		printError("Grid_Custom::FD_D2_N2, grids have not same size") ;
		return(RTN_CODE_KO) ;
	}
...
#pragma omp simd
	...
	printDebug(MID_DEBUG, "OUT Grid_Custom::FD_D2_N2");
	return(RTN_CODE_OK) ;
}

//-------------------------------------------------------------------------------------------------------

Rtn_code Grid_Custom::FD_D2_N3(Point_type pType, const Grid& Wgrid, Myint fdOrder)
{
	printDebug(MID_DEBUG, "IN Grid_Custom::FD_D2_N3");

	// check grids are same size
	if (this->sameSize(Wgrid) != true)
	{
		printError("Grid_Custom::FD_D2_N3, grids have not same size") ;
		return(RTN_CODE_KO) ;
	}
...
#pragma omp simd
	...
	printDebug(MID_DEBUG, "OUT Grid_Custom::FD_D2_N3");
	return(RTN_CODE_OK) ;
}

//-------------------------------------------------------------------------------------------------------

Rtn_code Grid_Custom::FD_LAPLACIAN(Point_type pType, const Grid& Wgrid, Myint fdOrder)
{
	printDebug(MID_DEBUG, "IN Grid_Custom::FD_LAPLACIAN");

	// check grids are same size
	if (this->sameSize(Wgrid) != true)
	{
		printError("Grid_Custom::FD_LAPLACIAN, grids have not same size") ;
		return(RTN_CODE_KO) ;
	}
...
#pragma omp simd
	...
	printDebug(MID_DEBUG, "OUT Grid_Custom::FD_LAPLACIAN");
	return(RTN_CODE_OK) ;
}

//-------------------------------------------------------------------------------------------------------

Rtn_code Grid_Custom::computePressureWithFD(Grid& prcGridIn, Grid& coefGridIn, Myint fdOrder)
{

	printDebug(FULL_DEBUG, "In Grid_Custom::computePressureWithFD") ;

	// check grids are same size
	if (this->sameSize(prcGridIn) != true)
	{
		printError("In Grid_Custom::computePressureWithFD, grids have not same size") ;
		return(RTN_CODE_KO) ;
	}
...
#pragma omp simd
	...
	printDebug(FULL_DEBUG, "Out Grid_Custom::computePressureWithFD") ;
	return(RTN_CODE_OK) ;
}
