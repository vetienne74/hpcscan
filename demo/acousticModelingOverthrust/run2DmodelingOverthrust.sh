...
# 2D acoustic modeling in the SEG/EAGE Overthrust model
...
# run hpcscan
mpirun -n 1 ../../bin/hpcscan -testCase Modeling -n1 $n1 -n2 $n2 -d1 $d1 -d2 $d2 \
		     -dim 2 -ntry 1 -tmax 2.0 -snapDt 0.25 -freqMax 20.0 \
		     -modelVpFile ${dataPath}/fvp \
		     -writeGrid

# display results
sh display2DResults.sh
